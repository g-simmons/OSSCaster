...
# c_path = '/data/Apachembox/Final_code/monthly_data/apache_commits/'
c_path = "../commits_raw/"
commits_p = os.listdir(c_path)
# e_path = '/data/Apachembox/Final_code/monthly_data/apache_emails/'
e_path = "../emails_raw/"
...
feature_names = ['active_devs','num_commits','num_committers','num_files','num_emails','num_senders','num_respondents',\
      'skew_c','skew_e','c_percentage','e_percentage','inactive_c','inactive_e',\
      'c_nodes','c_edges','c_c_coef','c_mean_degree','c_long_tail','c_triangles',\
      'e_nodes','e_edges','e_c_coef','e_mean_degree','e_long_tail','e_triangles', 'e_bidirected_edges']

nondata_cols = ['project','month', 'status']
for this_month in tqdm(range(1, 31)):
    csv_path = path + str(this_month) + '.csv'
    if not os.path.exists(csv_path):
        with open(csv_path, 'w') as f:
            # write the project, month and feature names
            f.write(','.join(nondata_cols + feature_names))
            f.write('\n')
    ...
    for project in projects:
        ...
        commits = [f for f in os.listdir(c_path) if project_id in f]
        emails = [f for f in os.listdir(e_path) if project_id in f]
        ...
        if max_month < this_month: continue
        ...
        for month in range(1, this_month+1):
            ...
            if int(project_id)<253:
                status = 'Graduated'
            else:
                status = 'Retired'

            with open(csv_path, 'a') as f:
                features = [project_id, str(month), status] + features
