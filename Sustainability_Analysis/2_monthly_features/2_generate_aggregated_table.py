...
def cal_tehnical_net(commit_path):
    ...
    committers = set(commit_dict.keys())
    num_core_devs = ceil(0.1 * len(committers))
    ...
    technical_features = [num_commits, committers, num_files, skew_c, c_percentage, inactive_c, \
...
def cal_social_net(email_path):
    ...
    senders = set(email_dict.keys())
    num_respondents = len(respondent_set)
    # core senders (top 10%)
    num_core_sender = ceil(0.1 * len(senders))
    ...
    social_net_features = [num_emails, senders, num_respondents, skew_e, e_percentage, inactive_e, \
            e_nodes, e_edges, e_c_coef, e_mean_degree, e_long_tail, e_triangles, e_bidirected_edges]

    return social_net_features


target_file = '../aggregated_features.csv'

with open(target_file, 'w') as f:
    things = ['project_id','incubation_length','active_devs','num_commits','num_committers','num_files','num_emails','num_senders','num_respondents',\
...
for project in tqdm(projects):
    ...
    commit_path = '../aggregated_data/commits/' + project_id + '.csv'
    email_path = '../aggregated_data/emails/' + project_id + '.csv' 

    if not os.path.exists(commit_path):
        inactive_c = 1
        committers = set()
        num_commits = num_files = skew_c = c_percentage = \
        c_nodes = c_edges = c_c_coef = c_mean_degree = c_long_tail = c_triangles = 0

    else:
        # compute commit features
        num_commits, committers, num_files, skew_c, c_percentage, inactive_c, \
        c_nodes, c_edges, c_c_coef, c_mean_degree, c_long_tail, c_triangles = cal_tehnical_net(commit_path)

    # computer email features
    if not os.path.exists(email_path):
        inactive_e = 1
        senders = set()
        num_emails = num_respondents = skew_e = e_percentage = \
        e_nodes = e_edges = e_c_coef = e_mean_degree = e_long_tail = e_triangles = e_bidirected_edges = 0
    
    else:
        num_emails, senders, num_respondents, skew_e, e_percentage, inactive_e, \
    ...
    active_devs = len(committers.union(senders))

    features = [incubation_length, active_devs,num_commits,num_committers,num_files,num_emails,num_senders,num_respondents, \
      skew_c,skew_e,c_percentage,e_percentage,inactive_c,inactive_e,\
      c_nodes,c_edges,c_c_coef,c_mean_degree,c_long_tail,c_triangles, \
      e_nodes,e_edges,e_c_coef,e_mean_degree,e_long_tail,e_triangles, e_bidirected_edges]
