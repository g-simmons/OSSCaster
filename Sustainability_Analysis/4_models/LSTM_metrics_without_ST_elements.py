...
metrics_path = "./results/metrics_without.csv"
...
for N_TIMESTEPS in tqdm(month_list):
    ...
    df = pd.read_csv(
        "../Reformat_data/{}.csv".format(N_TIMESTEPS)
    )
    ...
    data_columns = ["active_devs", "num_commits", "num_files", "num_emails"]
...
with open(metrics_path, "w") as f:
    ...
    for line in lines[1:]:
        ...
        f.write("{},w/o accuracy,{}\n".format(month, accuracy))
        f.write("{},w/o precision,{}\n".format(month, precision))
        f.write("{},w/o recall,{}\n".format(month, recall))
        f.write("{},w/o f1,{}".format(month, f1))
