...
def fit_CRN_decoder(dataset_train, dataset_val, model_name, model_dir,
    ...
    # length = len(dataset_train['project'])
    ...
    # num_inputs = len(dataset_train.keys())
...
def process_seq_data(data_map, states, projection_horizon):
    ...
    # sequence_lengths = np.repeat[len(data_map[0]), len(data_map.keys())]
    ...
    num_projects, num_time_steps, num_features = outputs.shape

    num_seq2seq_rows = num_projects * num_time_steps
    ...
    for i in range(num_projects):
        ...
        for t in range(1, sequence_length):  # shift outputs back by 1
            ...
            if seq2seq_active_entries[total_seq2seq_rows, :max_projection, :].shape != active_entries[i, t:t + max_projection, :].shape:
                # print(seq2seq_active_entries[total_seq2seq_rows, :max_projection, :].shape, active_entries[i, t:t + max_projection, :].shape)
                # print('Wrong')
                final_chunk = active_entries.shape[1] - t
                seq2seq_active_entries[total_seq2seq_rows, :final_chunk, :] = active_entries[i, t:t + final_chunk, :]
                seq2seq_previous_treatments[total_seq2seq_rows, :final_chunk, :] = previous_treatments[i, t - 1:t + final_chunk - 1, :]
                seq2seq_current_treatments[total_seq2seq_rows, :final_chunk, :] = current_treatments[i, t:t + final_chunk, :]
                seq2seq_outputs[total_seq2seq_rows, :final_chunk, :] = outputs[i, t:t + final_chunk, :]
                seq2seq_sequence_lengths[total_seq2seq_rows] = final_chunk
                seq2seq_current_covariates[total_seq2seq_rows, :final_chunk, :] = current_covariates[i, t:t + final_chunk, :]
            else:
...
def process_counterfactual_seq_test_data(test_data, data_map, states, projection_horizon):
    ...
    for i in range(num_patient_points):
        ...
        # print(seq2seq_previous_treatments[i].shape, previous_treatments[i, seq_length - 1:seq_length + projection_horizon - 1, :].shape)
