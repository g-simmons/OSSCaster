...
def get_processed_data(raw_sim_data,
    ...
    processed_data = {}
    for key in FEATURES:
        processed_data[key] = (raw_sim_data[key] - mean[key]) / std[key]
    ...
    # sequence_lengths = np.repeat[len(raw_sim_data['project']), len(raw_sim_data.keys())]
    ...
    treatments = raw_sim_data['treatments'][:, :-offset, np.newaxis]

    num_projects = len(raw_sim_data['prob_grad'])    # how many projects
    total_timesteps = len(raw_sim_data['prob_grad'][0])  # how many time steps
    one_hot_treatments = np.zeros(shape=(num_projects, treatments.shape[1], 5 * len(FEATURES)))
    for project_id in range(treatments.shape[0]):
        for timestep in range(treatments.shape[1]):
            # print(str(treatments[project_id][timestep]), str(treatments[project_id][timestep])[2:-1])
            feature, action = re.split('-', str(treatments[project_id][timestep])[3:-2])
            # print(str(treatments[project_id][timestep])[3:-2],)
            # pay attention to where to insert one-hot encoding
            one_hot_treatments[project_id][timestep][5 * FEATURES.index(feature) + ACTIONS.index(action)] = 1
    ...
    current_covariates = np.zeros(processed_data['active_devs'][:, :-offset, np.newaxis].shape)
    for key in processed_data.keys():
        if key != 'active_devs':
            current_covariates = np.concatenate(
                [current_covariates, processed_data[key][:, :-offset, np.newaxis]], axis=-1)  # all features
    # print("current_covariates.shape = ", current_covariates.shape)
    outputs = raw_sim_data['prob_grad'][:, horizon:, np.newaxis]
    # print('outputs shape in utils is: ', outputs.shape)

    output_means = mean[['prob_grad']].values.flatten()[0]  # because we only need scalars here
    output_stds = std[['prob_grad']].values.flatten()[0]
    ...
    #
    raw_sim_data['unscaled_outputs'] = (outputs * std['prob_grad'] + mean['prob_grad'])
